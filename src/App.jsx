        ...
        <div style={cardStyle()}>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(320px,1fr))", gap:14, alignItems:"start" }}>
            {calendars.map(({ monthKey, cells }) => (
              <div key={monthKey} style={{ border:"1px solid #e8edf3", borderRadius:18, padding:12, overflow:"hidden" }}>
                <div style={{ fontWeight:800, marginBottom:10, fontSize:16 }}>{monthLabels[monthKey]}</div>
                <div style={{ display:"grid", gridTemplateColumns:"repeat(7,minmax(0,1fr))", gap:4, marginBottom:6 }}>
                  {weekLabels.map((w) => (
                    <div key={w} style={{ fontSize:11, color:"#64748b", textAlign:"center", fontWeight:700, padding:"2px 0" }}>{w}</div>
                  ))}
                </div>
                <div style={{ display:"grid", gridTemplateColumns:"repeat(7,minmax(0,1fr))", gap:4 }}>
                  {cells.map((cell, idx) => {
                    if (!cell) return <div key={idx} style={{ minHeight: 58 }} />;
                    return (
                      <div
                        key={idx}
                        style={{
                      ...
                      >
                        <div style={{ fontSize:11, fontWeight:800, lineHeight:1 }}>{cell.day}</div>
                        <div style={{ marginTop:4, fontSize:12, fontWeight:800, color: cell.recommended <= 0 ? "#b91c1c" : "#0f172a", whiteSpace:"nowrap", overflow:"hidden", textOverflow:"ellipsis" }}>
                          {cell.recommended <= 0 ? "0 €" : shortEuros(cell.recommended)}
                        </div>
                        <div style={{ marginTop:2, fontSize:9, color:"#64748b", whiteSpace:"nowrap", overflow:"hidden", textOverflow:"ellipsis" }}>recomendado</div>
                      </div>
                    );
                  })}
                </div>
